...
- **ベースバージョン**：CalculiX **ccx 2.21**（`ccx_2.21.c` に `Version 2.21`、
  `http://www.dhondt.de/ccx_2.21.src.tar.bz2`）。
- 元のソースツリーでの位置は、ここでの相対パスと同じ（`ccx_2.21/src/...`）。

> **重要**：コピー方式 (B) は「フルの実ファイルで上書きする」ので、**上書き先が同じ ccx 2.21 で
> あること**が前提。別バージョンに上書きしないこと。バージョンが違う場合はパッチ方式 (A)
> （`patch -p1 < ...`）を使うか、まず 2.21 を用意すること。
