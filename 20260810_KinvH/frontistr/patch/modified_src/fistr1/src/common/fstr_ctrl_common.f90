...
contains
  ...
  function fstr_ctrl_get_SOLVER( ctrl, method, precond, nset, iterlog, timelog, steplog, nier, &
      ...
      dumptype, dumpexit, dumpw, usejad, ncolor_in, mpc_method, estcond, method2, recyclepre, &
    ...
    integer(kind=kint) :: iter, time, sclg, dmpt, dmpx, dmpw, usjd, step
    ...
    if( fstr_ctrl_get_param_ex( ctrl, 'DUMPW ',   'NO,YES ',           0,   'P',   dmpw ) /= 0) return
