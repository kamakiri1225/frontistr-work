...
$$\mathrm{Wdiff}=W_{\text{行0}}-W_{\text{行4}},\qquad W=K^{-1}H$$
...
#### 熱版: なぜPoint_Aの近くの節点で値が大きくなるのか

熱版は新しい物理を持ち出す必要がなく、力学版のKelvinの解と**相反定理**（Maxwell-Betti）で
説明できる。まず、アジョイント法の式を思い出す。

$$\mathrm{Wdiff}_j = (z_A-z_O)^T H_{:,j}, \qquad z_A=K^{-1}e_A,\ \ z_O=K^{-1}e_O$$

ここで $z_A$ は「Point_Aに単位の集中力を加えたときの構造全体の変位場」＝まさにKelvinの解の
離散版である。 $H_{:,j}$ は「節点 $j$ に単位温度を与えたときの等価節点力」で、節点 $j$ の
周りの要素にしか非ゼロ成分を持たない**局所的**なベクトルである。

$z_A^T H_{:,j}$ は「Point_A荷重の変位場」と「節点 $j$ の熱等価力」の内積だが、相反定理により
これは**「節点 $j$ に温度を与えたときにPoint_Aに生じる変位」**に等しい（これは $W_{A,j}$ の
定義そのものでもある）。つまりこの内積は、 $z_A$ を節点 $j$ の位置あたりで評価した値が
効いてくる。

**ピークが伸びる主因**: 節点 $j$ がPoint_Aに近いと、そこでの $z_A$ の値がKelvinの解に従って
${1/r}$ 的に大きい。メッシュを細かくするとPoint_Aのより近くに新しい節点が生まれるので、
その節点で拾う $z_A$ の値がより大きくなり、 $\mathrm{Wdiff}_j$ のピークが伸びる。

**ただし打ち消す効果もある（ここが元の説明で抜けていた点）**: $H_{:,j}$ の大きさは、
節点 $j$ が受け持つ体積（担当領域）にほぼ比例する。メッシュを細かくすると担当体積が
小さくなるので、 $H_{:,j}$ 自体は**小さくなる**方向に働く。したがって $\mathrm{Wdiff}_j$ の
ピークは「 $z_A$ の ${1/r}$ 的な増大」と「 $H_{:,j}$ の体積減少」の綱引きで決まり、
${1/r}$ がそのままピーク値の伸びになるわけではない。実測でも570→22123節点でピーク値は
約17倍で、節点数比（約39倍）や単純な ${1/r}$ より緩やかである。

**注意**: 上記は古典的な集中荷重解と相反定理を土台にした定性的な説明であり、
ピーク値がメッシュサイズ $h$ に対して具体的に何乗で伸びるかを、本プロジェクトの数値実験で
