...
念のため、342の色スケール上限を図1と同じ `4.7e-4`（左）と、その**0.5倍の `2.35e-4`**（右）に
した比較が次の図。上限を342自身のピークに合わせて半分にすると、342も341と**同じ場所
（柱の下部、Point_A/O近く）にホットスポットが出る**ことがはっきり見える＝分布の形は一次と
共通で、絶対値のスケールと基底が違うだけ、ということ。

![342を色スケール4.7e-4と0.5倍(2.35e-4)で比較](img/dumpw_wdiff_342_halfscale.png)
