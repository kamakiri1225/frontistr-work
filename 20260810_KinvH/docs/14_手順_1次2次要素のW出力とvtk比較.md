...
> **K・H はどのファイルのどのプログラムが計算しているか**（剛性 $K_e$＝`STF_C3`、温度荷重 $H_e$＝
> `TLOAD_C3`、全体組み立て＝`fstr_CreateMatrix.f90`＋`hecmw_mat_ass_elem`）と、その**計算式**は
> **付録A**（このドキュメント末尾）に、要素行列→全体行列の順でまとめてある。
